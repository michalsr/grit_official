...
def download_refclef(args):
    ...
    logging.info('Refclef downloaded')
...
def download_construction(args):
    ...
    logging.info('Construction downloaded')
...
def download_open_images(args):
    base = params.GRIT_IMAGES
    img_dir = f'{base}/open_images'
    mkdir_if_not_exists(img_dir,recursive=True)
    image_ids = set()
    for task in ['localization','categorization','segmentation']:
        base_path = params.GRIT_BASE
        grit_paths = GritPaths(f'{base_path}')
    ...
    #     shell=True)
    logging.info('Downloaded open images')
...
def download_visual_genome(args):
    base = params.GRIT_IMAGES
    img_dir = f'{base}/visual_genome'
    mkdir_if_not_exists(img_dir,recusrive=True)
    image_ids = set()
    grit_base = params.GRIT_BASE
    grit_paths = GritPaths(f'{grit_base}')
    for subset in ['ablation','test']:
    ...
    for image_id in image_ids:
        ...
        download_from_url(f'https://cs.stanford.edu/people/rak248/{image_id}',
            subdir)
    logging.info('Visual genome downloaded')
...
def main():
    ...
    parser.add_argument("--subsets_to_distort",nargs='+',default=None)
    ...
    for dataset in args.datasets:
        if dataset == 'coco':
            logging.info('Downloading coco')
            if not os.path.exists(f'{img_dir}/coco'):
                 download_coco(args)
            elif len(os.listdir(f'{img_dir}/coco')) == 0:
                download_coco(args)
            else:
                logging.info(f'COCO is already downloaded at {img_dir}/coco ')
        elif dataset == 'construction':
            logging.info('Downloading construction')
            if not os.path.exists(f'{img_dir}/construction_images'):
                download_construction(args)
            elif len(os.listdir(f'{img_dir}/construction_images')) == 0:
                download_construction(args)
            else:
                logging.info(f'Construction is already downloaded at {img_dir}/construction_images ')
            
        elif dataset == 'refclef':
            logging.info('Downloading refclef')
            if not os.path.exists(f'{img_dir}/saiapr_tc-12'):
                download_refclef(args)
            elif len(os.listdir(f'{img_dir}/saiapr_tc-12')) == 0:
                download_refclef(args)
            else:
                logging.info(f'Refclef is already downloaded at {img_dir}/saiapr_tc-12')
            
        elif dataset == 'open_images':
            logging.info('Downloading open images')
            if not os.path.exists(f'{img_dir}/open_images'):
                download_open_images(args)
            elif len(os.listdir(f'{img_dir}/open_images')) == 0:
                download_open_images(args)
            else:
                logging.info(f'Open images is already downloaded at {img_dir}/open_images')
        elif dataset == 'nyuv2':
            logging.info('Downloading NYUv2')
            if not os.path.exists(f'{img_dir}/nyuv2'):
                save_nyuv2(args)
            elif len(os.listdir(f'{img_dir}/nyuv2')) == 0:
                save_nyuv2(args)
            else:
                logging.info(f'NYUv2 is already downloaded at {img_dir}/nyuv2')
        elif dataset == 'visual_genome':
            logging.info('Downloading visual genome')
            if not os.path.exists(f'{img_dir}/visual_genome'):
                download_visual_genome(args)
            elif len(os.listdir(f'{img_dir}/visual_genome')) == 0:
                download_visual_genome(args)
            else:
                logging.info(f'Visual genome is already downloaded at {img_dir}/visual_genome')
        elif dataset == 'blended_mvs':
            logging.info('Downloading Blended MVS')
            if not os.path.exists(f'{img_dir}/blended_mvs'):
                download_blended_mvs(args)
            elif len(os.listdir(f'{img_dir}/blended_mvs')) == 0:
                download_blended_mvs(args)
            else:
                logging.info(f'Blended MVS is already downloaded at {img_dir}/blended_mvs')
        elif dataset == 'dtu':
            logging.info('Downloading DTU')
            if not os.path.exists(f'{img_dir}/dtu_images'):
                download_dtu(args)
            elif len(os.listdir(f'{img_dir}/dtu_images')) == 0:
                download_dtu(args)
            else:
                logging.info(f'DTU is already downloaded at {img_dir}/dtu_images')
